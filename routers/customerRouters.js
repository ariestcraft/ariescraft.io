...
Routers.get('/customer/uploaded_files', async function (req, res) {
    ...
    if (req.session.customerLogin) {
        ...
        await getData("SELECT * FROM complete_project WHERE product_id =" + req.query.id + " AND client_id =" + req.session.customerId + "").then((data) => {
            project = data;
        }).catch((err) => console.error(err));

        res.render(__dirname + '/customer/uploaded_files', { project: project });
    } else {
...
Routers.get('/customer/view_product', async function (req, res) {
    ...
    if (req.session.customerLogin) {
        ...
        res.render(__dirname + '/customer/view_product', { product: product });
    } else {
        //res.send('Please login to view this page!');
        req.session.destroy();
        res.render(__dirname + '/customer/login');
